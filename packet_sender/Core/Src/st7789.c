...
/**
 * @brief Write single byte of data to ST7789 controller
 */
 ...
 * @param m -> rotation parameter 0-3
 */
...
void ST7789_Init(void)
{
	...
	/* Software reset — mandatory, clears all registers */
	ST7789_WriteCommand(ST7789_SWRESET);
	HAL_Delay(150);		/* Datasheet requires 120ms min after SWRESET */
	...
	/* Inversion ON (required for correct colors on most ST7789 modules) */
	ST7789_WriteCommand(ST7789_INVON);

	/* Exit sleep — must wait 120ms before sending more commands */
...
/**
 * @brief Draw a 3x3 pixel block at a point
 */
...
/**
 * @brief Draw an image from a pixel buffer
 */
...
/**
 * @brief Enable/disable tearing effect line
 */
...
/**
 * @brief Display test sequence
 */
void ST7789_Test(void)
{
	ST7789_Fill_Color(WHITE);   HAL_Delay(500);
	ST7789_Fill_Color(RED);     HAL_Delay(500);
	ST7789_Fill_Color(GREEN);   HAL_Delay(500);
	ST7789_Fill_Color(BLUE);    HAL_Delay(500);
	ST7789_Fill_Color(BLACK);   HAL_Delay(500);

	ST7789_WriteString(10, 10, "ST7789 OK", Font_11x18, WHITE, BLACK);
	ST7789_WriteString(10, 40, "240x240 RGB", Font_11x18, YELLOW, BLACK);
	HAL_Delay(2000);
	...
	ST7789_Fill_Color(BLACK);
	ST7789_DrawCircle(120, 120, 50, GREEN);
	HAL_Delay(1000);

	ST7789_Fill_Color(BLACK);
	ST7789_DrawFilledCircle(120, 120, 50, CYAN);
	HAL_Delay(1000);
}
